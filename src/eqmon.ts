  ...
  async playJingleAndGenerateVoice(data: EventData): Promise<void> {
    switch (data.code) {
      case 551: {
        ...
        const eqTime = jmaQuake.earthquake.time;
        ...
        if (this.lastJMAQuake && this.lastJMAQuake.eqTime === eqTime && this.lastJMAQuake.pref === pref && this.lastJMAQuake.scale === scale) {
          return;
        }

        this.playSound(join(PROJECT_ROOT, "wav/info.wav"));
        const [voicePath] = await Promise.all([generateJMAQuakeVoice(pref, scale), sleep(1000)]);
        this.playSound(voicePath);
        this.lastJMAQuake = { eqTime, pref, scale };
      ...
      case 9611: {
        ...
        const started_at = uqe.started_at;

        if (this.lastUqe && this.lastUqe.started_at === started_at) {
          return;
        }
